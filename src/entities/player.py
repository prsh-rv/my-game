    ...
    def set_mode(self, mode: PlayerMode) -> None:
        ...
        if mode == PlayerMode.NORMAL:
            ...
            if self.crash_entity == "pipe":
                # play a random die sound from the configured list
                try:
                    self.config.sounds.play_die()
                except Exception:
                    # fallback to single die if play_die not available
                    if getattr(self.config.sounds, "die", None):
                        try:
